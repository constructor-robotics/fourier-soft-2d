...
void PeakFinder::findPeaks(std::vector<float> x0, std::vector<int> &peakInds, bool includeEndpoints, float impactOfNoise, float extrema) {
    int minIdx = distance(x0.begin(), min_element(x0.begin(), x0.end()));
    int maxIdx = distance(x0.begin(), max_element(x0.begin(), x0.end()));

    float sel = (x0[maxIdx] - x0[minIdx]) / impactOfNoise;
    int len0 = x0.size();
...
indexPeak findMaxNeighbor(double x0[], int x, int y, int dimSize) {
    indexPeak maxIndex;
    maxIndex.x = x;
    maxIndex.y = y;
    for (int i = -1; i < 2; i++) {
        for (int j = -1; j < 2; j++) {
            ...
            if (!(xIndex < 0 || xIndex >= dimSize || yIndex < 0 || yIndex >= dimSize)) {
                if (x0[yIndex + dimSize * xIndex] > x0[maxIndex.y + dimSize * maxIndex.x]) {
                    maxIndex.x = xIndex;
                    maxIndex.y = yIndex;
                }
            }
...
//THIS IS NOT PERFECT IT IGNORES SOME LOCAL MAXIMA. BUT VERY LIKELY UNIMPORTANT ONE,SINCE BIG LOCAL MAX NEARBY
    ...
    while (true) {
        int sizeBefore = potentialPeaks.size();
        std::vector<indexPeak> newPotentialPeaks;
        for (int i = 0; i < potentialPeaks.size(); i++) {
            //for each peak find maximum neighbor
            indexPeak maxInlocalArea = findMaxNeighbor(x0, potentialPeaks[i].x, potentialPeaks[i].y, dimSize);
            newPotentialPeaks.push_back(maxInlocalArea);
        }

        //sort out multiple expenses
        std::vector<indexPeak> uniquePotentialPeaks;
        for (int i = 0; i < newPotentialPeaks.size(); i++) {
            //for each peak find maximum neighbor
            bool isUnique = true;
        ...
        int sizeAfter = uniquePotentialPeaks.size();
        potentialPeaks = uniquePotentialPeaks;
        if (sizeBefore == sizeAfter) {
            //potentialPeaks.clear();

            for (int i = 0; i < potentialPeaks.size(); i++) {
                potentialPeaks[i].height = x0[potentialPeaks[i].y + dimSize * potentialPeaks[i].x];
            }
            break;
        }
    }
